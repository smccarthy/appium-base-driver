...
class BaseDriver extends MobileJsonWireProtocol {
  ...
  resetOnUnexpectedShutdown() {
    // TODO: use positive logic (i.e. onShutdown)
  ...
  validateLocatorStrategy (strategy, webContext = false) {
    ...
    let validStrats = this.locatorStrategies;

    if (webContext) {
      validStrats = validStrats.concat(this.webLocatorStrategies);
    }

    if (!_.contains(validStrats, strategy)) {
